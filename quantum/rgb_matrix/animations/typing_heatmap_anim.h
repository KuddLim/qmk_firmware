...
RGB_MATRIX_EFFECT(TYPING_HEATMAP_KUDD)
...
void process_rgb_matrix_typing_heatmap(uint8_t row, uint8_t col, uint8_t original) {
...
bool TYPING_HEATMAP_KUDD(effect_params_t* params) {
    RGB_MATRIX_USE_LIMITS(led_min, led_max);

    if (params->init) {
        rgb_matrix_set_color_all(0, 0, 0);
        memset(g_rgb_frame_buffer, 0, sizeof g_rgb_frame_buffer);
        memset(g_rgb_frame_buffer2, 0, sizeof g_rgb_frame_buffer2);
        memset(g_rgb_frame_buffer3, 0, sizeof g_rgb_frame_buffer3);
    }
    ...
    for (uint8_t row = 0; row < MATRIX_ROWS && count < RGB_MATRIX_LED_PROCESS_LIMIT; row++) {
        for (uint8_t col = 0; col < MATRIX_COLS && RGB_MATRIX_LED_PROCESS_LIMIT; col++) {
            if (g_led_config.matrix_co[row][col] >= led_min && g_led_config.matrix_co[row][col] < led_max) {
                ...
                //HSV hsv = {170 - qsub8(val, 85), rgb_matrix_config.hsv.s, scale8((qadd8(170, val) - 170) * 3, rgb_matrix_config.hsv.v)};
                HSV hsv = {g_rgb_frame_buffer2[row][col], g_rgb_frame_buffer3[row][col], scale8((qadd8(170, val) - 170) * 3, rgb_matrix_config.hsv.v)};
                RGB rgb = rgb_matrix_hsv_to_rgb(hsv);
                rgb_matrix_set_color(g_led_config.matrix_co[row][col], rgb.r, rgb.g, rgb.b);

                if (decrease_heatmap_values) {
                    g_rgb_frame_buffer[row][col] = qsub8(val, 5);
                }
            }
        }
    }
